...
//	protected $cst_degrade ='~\[degrade=(\d*?),(\d*?),(\d*?),(\d*?),([[:alnum:]]*?),([[:alnum:]]*?),([[:alnum:]]*?)\]~s' ;
...
//	private function degrade($x1,$x2, $y1, $y2,$direction,$color1,$color2)
//	{
//		$color1 = array_values(imagecolorsforindex($this->img,$color1));
//		$color2 = array_values(imagecolorsforindex($this->img,$color2));
		
		
		 		//$size = imagesx($this->img);
//             	$size = $x2 - $x1;
               // $sizeinv = imagesy($this->img);
//                $sizeinv = $y2 - $y1;
...
//			$b = $color1[2]+($diffs[2]*$i);
	...
	//		imageline($this->img,$i + $x1 ,$y1,$i +$x1 , $y2,imagecolorallocate($this->img,$r,$g,$b));
//			imageline($this->img, $x1 ,$y1,$x2 , $y2,imagecolorallocate($this->img,$r,$g,$b));
...
//		$this->get_degrade(); //
	...
	//	$colora= imagecolorallocate($this->img, 180, 180, 180);
	//	$colorb= imagecolorallocate($this->img, 50, 50, 50);
		
	//	$a = array_values(imagecolorsforindex($this->img, $this->color["color1"]));
	//	$b =  array_values(imagecolorsforindex($this->img, $this->color["color2"]));
	...
	private function get_degrade()
	{
		...
		if 	(preg_match_all($this->cst_degrade ,  $this->code, $matches))
		{
			...
			for ($i = 0; $i < $total; $i++) {
	//			$this->degrade($matches[1][$i],$matches[2][$i],$matches[3][$i],$matches[4][$i],$matches[5][$i],$this->color[$matches[6][$i]],$this->color[$matches[7][$i]]);
	...
	//		var_dump($matches);
	//		var_dump($this->color[$matches[7][0]]);
	//		var_dump($this->color[$matches[6][0]]);
